...
('a444bae9-3193-465c-853b-9528abe1023e', 'Alpha Association', 'Association humanitaire française active dans l''aide d''urgence, l''action sociale et la formation aux premiers secours.', (SELECT id FROM organization_sectors WHERE name = 'Humanitaire'), '98 Rue de Rivoli, 75001 Paris', 'https://www.alpha-association.fr', '/logos/alpha.png', '77567227900224', '2020-01-15', 48.8606, 2.3376, now(), now())
...
-- D'abord on récupère l'ID de l'organisation Alpha
DO $$
DECLARE
    alpha_org_id uuid;
    mission_alpha_01 uuid := gen_random_uuid();
    mission_alpha_02 uuid := gen_random_uuid();
    mission_alpha_03 uuid := gen_random_uuid();
    mission_alpha_04 uuid := gen_random_uuid();
BEGIN
    SELECT id INTO alpha_org_id FROM organization_profiles WHERE user_id = 'a444bae9-3193-465c-853b-9528abe1023e';
    ...
    IF alpha_org_id IS NOT NULL THEN
        ...
        (gen_random_uuid(), mission_alpha_01, (SELECT id FROM skills WHERE name = 'Communication'), 'débutant', false, now()),
        (gen_random_uuid(), mission_alpha_01, (SELECT id FROM skills WHERE name = 'Organisation'), 'débutant', false, now()),

        -- Formation premiers secours - Compétences requises
        (gen_random_uuid(), mission_alpha_02, (SELECT id FROM skills WHERE name = 'Premiers secours'), 'avancé', true, now()),
        (gen_random_uuid(), mission_alpha_02, (SELECT id FROM skills WHERE name = 'Communication'), 'avancé', true, now()),

        -- Accompagnement scolaire - Animation recommandée
        (gen_random_uuid(), mission_alpha_03, (SELECT id FROM skills WHERE name = 'Animation'), 'intermédiaire', false, now()),
        (gen_random_uuid(), mission_alpha_03, (SELECT id FROM skills WHERE name = 'Communication'), 'intermédiaire', true, now())
        ...
        (gen_random_uuid(), 'ea44982b-ba08-45cd-b051-255541c38bff', mission_alpha_01, 'confirmé', now() - interval '2 days', now() - interval '1 day', null, null, null, null, now() - interval '2 days', now() - interval '1 day'),

        -- Ben inscrit à l'accompagnement scolaire (en attente)
        (gen_random_uuid(), 'ea44982b-ba08-45cd-b051-255541c38bff', mission_alpha_03, 'inscrit', now() - interval '1 day', null, null, null, null, null, now() - interval '1 day', now() - interval '1 day')
        ...
        (gen_random_uuid(), 'ea44982b-ba08-45cd-b051-255541c38bff', 'Mission confirmée !', 'Votre participation à "Distribution alimentaire hebdomadaire" a été confirmée.', false, '/missions/' || mission_alpha_01, now() - interval '1 day'),
        ...
        (gen_random_uuid(), 'a444bae9-3193-465c-853b-9528abe1023e', 'Nouvelle inscription', 'Ben s''est inscrit à votre mission "Distribution alimentaire".', false, '/missions/' || mission_alpha_01, now() - interval '2 days'),
        (gen_random_uuid(), 'a444bae9-3193-465c-853b-9528abe1023e', 'Mission bientôt complète', 'Il ne reste que 3 places pour "Formation premiers secours".', false, '/missions/' || mission_alpha_02, now() - interval '1 day')
...
SELECT 
    ...
    (SELECT COUNT(*) FROM missions WHERE organization_id = (SELECT id FROM organization_profiles WHERE user_id = 'a444bae9-3193-465c-853b-9528abe1023e')) as nb_missions_alpha,
